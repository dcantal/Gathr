...
grandcentral = Event.create(
    name: "Grand Central Scavenger & History Hunt", 
    address: "Grand Central Terminal, East 42nd Street, New York, NY",
    description: "Kick off your weekend with dozens of Meetup members for a fun Happy Hour with a twist! The Great Northern Food hall at Grand Central is a relatively new addition to the iconic building located on the street level that features 2 bars and several food stands. Join us for Happy Hour cocktails then set out into Grand Central for a fun scavenger hunt where you will learn the history of the building and discover many things you may never have noticed.
...
Each team will have 75 minutes to complete the scavenger hunt with a list of questions, challenges, and puzzles. The teams with the most points will win prizes. Level? No prior knowledge needed of anything… just have fun.", 
    group_id: rft.id, 
    start_time: DateTime.new(2019,6,17,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,17,22,0,0,Rational(-4,24)), 
    latitude: 40.750935, 
    longitude: -73.977104, 
    private: FALSE)

speedfriend = Event.create(
    name: "Speed FRIENDING (40s & Over)", 
    address: "BRYANT PARK LOUNGE, 6th Avenue, New York, NY", 
    description: "If you are in your 40s or over, looking to expand your social circle and make new friends/activity partners, then this Meetup is for you!
...
Is this age-specific? Yes, this is a special edition of our Speed Friending intended for 40s, 50s +", 
    group_id: rft.id, 
    start_time: DateTime.new(2019,6,18,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,18,23,0,0,Rational(-4,24)), 
    latitude: 40.752865, 
    longitude: -73.98599, 
    private: FALSE)

murdermystery = Event.create(
    name: "Murder Mystery Mixer - Everyone is a suspect!", 
    address: "World Bar, United Nations Plaza, New York, NY", 
    description: "Our Murder Mystery Parties have become an awesome alternative to the usual bar scene. It is a great way to go out on your own with friends or on your own and meet other people in a super organized and fun way! We will also have great drink specials!
...
For weeks, people have been pouring into the small western town of Deadwood for the biggest poker tournament this side of the Mississippi. From outlaws to marshals and saloon girls to southern belles, everyone has made this small western frontier town THE place to be. As the festivities of the poker tournament conclude, a celebration party is planned at the Deadwood Saloon to award the prize money for the tournament and to celebrate what is hoping to be a very financially successful venture for the saloon.", 
    group_id: rft.id, 
    start_time: DateTime.new(2019,6,25,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,25,23,0,0,Rational(-4,24)), 
    latitude: 40.7525175, 
    longitude: -73.969879, private: FALSE)

springyacht = Event.create(
    name: "Spring Yacht Party Cruise (DJ, Comp Appetizers, Ice Breakers, Prizes)", 
    address: "2430 Fdr Drive Service Road East, New York, NY", 
    description: "Members keep asking when are we doing the Yacht Cruise Meetup again... well, here it is! Join 200+ singles (ages 20s-40s) from multiple groups & set sail around Manhattan for a 4 hour private tour & party!
...
This is a multi-Meetup and very popular event! It will SELL OUT so if you'd like to attend, please get your ticket ASAP.", 
    group_id: rft.id, 
    start_time: DateTime.new(2019,6,26,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,26,23,0,0,Rational(-4,24)), 
    latitude: 40.73804, 
    longitude: -73.973656, 
    private: FALSE)

machinelearning = Event.create(
    name: "Machine learning: Trends, perspectives, and prospects @ Attune", 
    address: "40 Exchange Pl, New York, NY 10005", 
    description: "Come and learn for an exciting talk about Machine Learning! This exciting space will only grow bigger in the future and you should definitely add this knowledge to your repertoire ", 
    group_id: wwc.id, 
    start_time: DateTime.new(2019,6,17,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,17,22,0,0,Rational(-4,24)), 
    latitude: 40.7057883, 
    longitude: -74.0102498, 
    private: FALSE)

algorithms = Event.create(
    name: "Algorithms@Betterment", 
    address: "61 W 23rd St, New York, NY 10010", 
    description: "Come for some algorithms practice ", 
    group_id: wwc.id, 
    start_time: DateTime.new(2019,6,24,20,0,0,Rational(-4,24)), end_time: DateTime.new(2019,6,24,22,0,0,Rational(-4,24)), latitude: 40.7426416, longitude: -73.9939997, private: FALSE)

maynytech = Event.create(
    name: "June 2019 NY Tech Meetup", 
    address: "28 W 23rd St, New York, NY 10010", 
    description: "Join us for one of NYC's most famous and longest running monthly tech event! You'll see a fantastic lineup of New York tech companies presenting live demos of their products, followed by an afterparty where you can network with the community and meet our demoers and sponsors.
...
This month, we are going farm to table with a great lineup of Food/ Agriculture/ Restaurant tech! Stay tuned for the full list of companies.", 
    group_id: nytm.id, 
    start_time: DateTime.new(2019,6,24,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,24,22,0,0,Rational(-4,24)), 
    latitude: 40.741898, 
    longitude: -73.99055, 
    private: FALSE)

junenytech = Event.create(
    name: "July 2019 NY Tech Meetup", 
    address: "28 W 23rd St, New York, NY 10010", 
    description: "Join us for one of NYC's most famous and longest running monthly tech event! You'll see a fantastic lineup of New York tech companies presenting live demos of their products, followed by an afterparty where you can network with the community and meet our demoers and sponsors.", 
    group_id: nytm.id, 
    start_time: DateTime.new(2019,7,24,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,7,24,22,0,0,Rational(-4,24)), 
    latitude: 40.741898, 
    longitude: -73.99055, 
    private: FALSE)

thirdsocial = Event.create(
    name: "Third social gathering", 
    address: "148 W 24th St, New York, NY 10011", 
    description: "This will be the third social get together to discuss games over drinks. The poll was pretty conclusive that Barcade is the favored spot. I’ll leave the poll up, for those who want to suggest other places. I’m going to see if we get any more RSVPs tomorrow, but if not, will prob postpone - this might be a bad week, as I understand some might be on spring break and Wednesday’s may not work well.", 
    group_id: game.id, 
    start_time: DateTime.new(2019,6,17,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,17,22,0,0,Rational(-4,24)), latitude: 40.7443119, longitude: -73.9944248, 
    private: FALSE)

fourthsocial = Event.create(
    name: "Fourth social gathering", 
    address: "148 W 24th St, New York, NY 10011", 
    description: "This will be the fourth social get together to discuss games over drinks. The poll was pretty conclusive that Barcade is the favored spot. I’ll leave the poll up, for those who want to suggest other places. I’m going to see if we get any more RSVPs tomorrow, but if not, will prob postpone - this might be a bad week, as I understand some might be on spring break and Wednesday’s may not work well.", 
    group_id: game.id, 
    start_time: DateTime.new(2019,6,24,20,0,0,Rational(-4,24)), 
    end_time: DateTime.new(2019,6,24,22,0,0,Rational(-4,24)), latitude: 40.7443119, longitude: -73.9944248, 
    private: FALSE)
